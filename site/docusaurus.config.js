...
import localeFlags from './plugins/locale-flags.js';
import {DEFAULT_LOCALE, SUPPORTED_LOCALES, localeConfigs, lunrLanguages} from './src/locales.js';
...
const config = {
  ...
  // Locales are declared in src/locales.js, which is this repository's only copy of the set, and the
  // docs plugin falls back to the English file whenever a translated one is absent. That fallback is
  // what makes a partial translation shippable: an untranslated page renders in English rather than
  // 404ing, so a locale can go live at any coverage. It is also why staleness is invisible, which is
  // what `cd ../../epic-i18n && npm run verify` exists to report.
  //
  ...
  //
  // The workspace registry at locales.json is the source of truth for the set; src/locales.js is a
  // checked copy of it, because a standalone clone may not reach outside this tree. Prove they agree
  // with `node tools/locales.mjs check` from the workspace root.
  i18n: {
  ...
  themes: [
    [
      ...
      ({
        hashed: true,
        // One stemmer key per locale, from src/locales.js. It is a different vocabulary from the
        // locale key: zh-CN maps to zh. 'ru' selects the Russian stemmer, so a query for "кошелька"
        // matches "кошелёк". 'zh' selects the Chinese tokenizer, without which CJK text never segments
        // into terms at all and the index is not merely worse for Chinese, it is empty of useful
        // entries. Costs no new dependency: @node-rs/jieba is already a hard dependency of this theme.
        language: lunrLanguages(),
        indexBlog: false,
  ...
  plugins: [assistantDevProxy, localeFlags],
    ...
    ({
      ...
      navbar: {
        ...
        items: [
          ...
          // `className` lands on the control that opens the dropdown, which is the hook custom.css
          // uses to swap the language name for the current locale's flag. It is here rather than a
          // `.navbar__item.dropdown` selector so the CSS keeps working if a second dropdown is ever
          // added to the navbar.
          {type: 'localeDropdown', position: 'right', className: 'epicLocalePicker'},
