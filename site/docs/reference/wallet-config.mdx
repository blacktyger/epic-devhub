...
| `api_secret_path` | string, optional | `<wallet dir>/.owner_api_secret` | Basic auth secret presented on the owner listener. Note that the middleware on this listener is built for the Foreign path, so this is not what guards `/v3/owner`; the wallet password is. See [authentication](/api/authentication) |
